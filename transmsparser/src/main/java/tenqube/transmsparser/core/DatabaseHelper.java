        ...
        if(db!=null){
            ...
            try {
                ...
                while ((line = buffer.readLine()) != null) {
                    ...
                    if(values.size()==200) {
                        db.execSQL(ParserReaderContract.RegExpressionTable.INSERT_REG+ TextUtils.join(",", values));
                        values.clear();
                    }
                ...
                LogUtil.LOGI(TAG, "inserted parsing rule size: " + i);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void insertSenders(SQLiteDatabase db) throws SQLiteException{
        ...
        if(db!=null) {
            ...
            try {
                ...
                if(!values.isEmpty()) {
                    String insertQuery = ParserReaderContract.SendersTable.INSERT_SENDER + TextUtils.join(",", values);
    ...
    private void insertBanks(SQLiteDatabase db) throws SQLiteException{
        ...
        if(db!=null) {
            ...
            try {
                ...
                if(!values.isEmpty()) {
                    String insertQuery = ParserReaderContract.BanksTable.INSERT_BANK + TextUtils.join(",", values);
                    db.execSQL(insertQuery);
                    values.clear();
                }
