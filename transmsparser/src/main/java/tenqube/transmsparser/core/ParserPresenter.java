...
class ParserPresenter {
    ...
    ArrayList<Transaction> parseWithBulk(SMS sms) {
        ...
        LogUtil.LOGI(TAG, "0. sender 대표 필터" + repSender);
        if (repSender == null)
            return null;

        if(isExistTranWithBulk(sms.getSmsId(), sms.getSmsType())) {
            LogUtil.LOGI(TAG, "isExistTranWithBulk: 이미 존재");
            return null;
        }
        ...
        if (transaction == null) {
            LogUtil.LOGI(TAG, "1. 정규표현식을 통한 데이터 확인: FALSE");
            LogUtil.LOGI(TAG, "NOT PARSED: "+sms.toString());
            return null;
        }

        LogUtil.LOGI(TAG, "1. 정규표현식을 통한 데이터 확인: TRUE" + transaction.toString());
        //2. 중복 데이터 확인
        LogUtil.LOGI(TAG, "2. 중복 데이터 확인");
        TransactionTableData duplicationTran = getDuplicationTranWithBulk(transaction);

        //중복
        if (duplicationTran != null) {

            LogUtil.LOGI(TAG, "2. 중복 데이터 확인: TRUE" + duplicationTran.toString());
            ...
            //중복 아님
            LogUtil.LOGI(TAG, "2. 중복 데이터 확인: FALSE");

            //상쇄 확인
            offsetTran = getOffsetTranWithBulk(transaction);
            LogUtil.LOGI(TAG, "3. 중복 x 상쇄 Tran 확인");

            //상쇄
            if (offsetTran != null) {

                LogUtil.LOGI(TAG, "3. 중복 x -> 상쇄 Tran 확인: TRUE" + offsetTran.toString());
                transaction = mRegHandler.combineTranWithOffset(offsetTran, transaction);


            } else {
                //상쇄아님
                LogUtil.LOGI(TAG, "3. 중복 x -> 상쇄 Tran 확인: FALSE");

                //자산이동 확인
                LogUtil.LOGI(TAG, "4. 중복 x -> 상쇄 x -> 자산이동 Tran 확인");

                movingAssetTran = getMovingAssetTranWithBulk(transaction);

                //자산이동
                if (movingAssetTran != null) {
                    LogUtil.LOGI(TAG, "4. 중복 x 상쇄 x 자산이동 Tran 확인: TRUE" + movingAssetTran.toString());
                    ...
                    LogUtil.LOGI(TAG, "4. 중복 x 상쇄 x 자산이동 Tran 확인: FALSE");
        ...
        LogUtil.LOGI(TAG, "최종 리턴 사이즈:" + resultTrans.size());
        return resultTrans;
    }
...
//                    Collections.sort(assetTrans, new Utils.TranComparator());
    ...
    ParserResult parse(SMS sms) {
        ...
        try {
            ...
            //1. 정규표현식을 통한 데이터 확인
            LogUtil.LOGI(TAG, "1. 정규표현식을 통한 데이터 확인");
